...
function* axiosCommentList(action) {
    try {
        // 正常返回之后，数据直接给res     
        const res = yield axios.get('/comment/test/commentlist',{params:{blogid: action.blogid,page: 0}});
...
function* axiosLoadMoreComment(action){
    try {   
        const res = yield axios.get('/comment/test/commentlist',{params:{page:action.page,blogid: action.blogid}});
        if( res.data.length < 3){
            yield put(changeHasCommentToFalse());
        }
...
function* axiosSubmitComment(action){
    try{
        ...
        if(action.name.trim()===""){
            ...
            const res = yield axios.post('/comment/test/addcomment',{name: action.name, mail: action.mail, comment: action.comment.replace(/'/g,'"'), blogid: action.blogid})
